...
func TestUpdateStatusProduct_Execute(t *testing.T) {
	t.Parallel()
	...
	t.Run("ok", func(t *testing.T) {
		...
		repo.EXPECT().
			UpdateStatus(gomock.Any(), id, domain.ProductStatusActive).
			Return(testhelpers.RestoredProduct(id), nil)

		uc := v1usecase.NewUpdateStatusProduct(repo)
		got, err := uc.Execute(context.Background(), v1usecase.UpdateStatusProductCommand{
			ID:     id,
	...
	t.Run("not_found", func(t *testing.T) {
		...
		uc := v1usecase.NewUpdateStatusProduct(repo)
		_, err := uc.Execute(context.Background(), v1usecase.UpdateStatusProductCommand{
			ID:     id,
			Status: domain.ProductStatusActive,
		}).ToGo()
	...
	t.Run("repository_error", func(t *testing.T) {
		...
		repo := mocks.NewMockProductRepository(ctrl)
		repo.EXPECT().
			UpdateStatus(gomock.Any(), id, domain.ProductStatusInactive).
